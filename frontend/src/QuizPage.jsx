...
import { FaCheckCircle, FaTimesCircle, FaHome, FaCog } from "react-icons/fa";
...
const TypingEffect = ({ text, speed = 30 }) => {
  ...
  useEffect(() => {
    // Clear displayedText instantly when text prop changes to reset typing
    setDisplayedText(''); 
    if (!text) {
      return;
    }
...
// Helper function to get audience description based on classId
const getAudienceDescription = (classId) => {
  switch (classId) {
    case '1': return 'a Primary 1 student (age 6-7)';
    case '2': return 'a Primary 2 student (age 7-8)';
    case '3': return 'a Primary 3 student (age 8-9)';
    case '4': return 'a Primary 4 student (age 9-10)';
    case '5': return 'a Primary 5 student (age 10-11)';
    case '6': return 'a Primary 6 student (age 11-12)';
    case '7': return 'a Junior Secondary School (JSS 1) student (age 12-13)';
    case '8': return 'a Junior Secondary School (JSS 2) student (age 13-14)';
    case '9': return 'a Junior Secondary School (JSS 3) student (age 14-15)';
    // Add more cases as needed for your specific class IDs
    default: return 'a general audience with simple, clear language'; // Default for higher classes or unknown
  }
};
...
const QuizPage = () => {
  ...
  // Get the audience description for the AI
  const audienceDescription = getAudienceDescription(classId);
  ...
  const fetchAiExplanation = useCallback(async () => {
    ...
    try {
      // Changed the fetch URL to a relative path for Vercel
      const response = await fetch('/api/generate-explanation', {
        ...
        body: JSON.stringify({
          ...
          options: currentQuestion.options.map(opt => ({ text: opt })),
          correctAnswer: currentQuestion.correctAnswer,
          targetAudience: audienceDescription, // Pass the audience description to the backend
        }),
      });
  ...
  }, [currentQuestion, audienceDescription]); // Added audienceDescription to dependencies
  ...
  const handleOptionClick = (option) => {
    ...
    setIsTyping(false); // Reset typing effect
  };
                    ...
                    <div className="explanation-text">
                      {loadingAiDeepExplanation ? (
                        ...
                        <ReactMarkdown remarkPlugins={[remarkGfm]}>
                          <TypingEffect text={aiDeepExplanation} speed={25} />
                        </ReactMarkdown>
                      )}
                    </div>
                  </div>
