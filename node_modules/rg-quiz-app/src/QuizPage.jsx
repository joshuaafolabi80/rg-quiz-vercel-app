    ...
    const QuizPage = () => {
      ...
      const fetchAiExplanation = useCallback(async () => {
        ...
        try {
          // Ensure this URL matches your local backend server (e.g., http://localhost:3001)
          ...
          if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Failed to fetch AI explanation from backend. Server responded with an error.');
          }

          const data = await response.json();
          console.log("Received raw data from backend:", data); // Log the full response to inspect its structure

          // Check if data.explanation is a string before setting it
          if (data && typeof data.explanation === 'string') {
            setAiDeepExplanation(data.explanation);
            console.log("AI explanation successfully set:", data.explanation);
          } else {
            // Fallback for unexpected response formats
            console.error("Backend response did not contain a string 'explanation' property:", data);
            setAiDeepExplanationError("Failed to get AI explanation: Unexpected response format from server.");
            setAiDeepExplanation("Error: Could not display explanation due to unexpected format.");
          }

        } catch (error) {
          console.error('Error fetching AI explanation:', error);
          setAiDeepExplanationError(`Failed to get AI explanation: ${error.message}`);
          setAiDeepExplanation("Error: Could not retrieve explanation. Please check console for details.");
        } finally {
                      ...
                      <div className="ai-explanation-box mt-2 p-1 bg-white rounded">
                        ...
                        <div className="explanation-text">
                          {loadingAiDeepExplanation ? (
                            ...
                            // HERE IS THE CHANGE: ReactMarkdown will render the full aiDeepExplanation string
                            // The TypingEffect component is removed from here.
